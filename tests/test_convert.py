...
class TestConversion:
    ...
    def test_writes_into_the_chosen_folder_creating_it(self, make_pdf, tmp_path):
        ...
        result = convert(pdf, tmp_path / "converted" / "2026")
    ...
    def test_the_extension_does_not_matter(self, make_pdf, tmp_path):
        ...
        assert convert(pdf).output == tmp_path / "SCAN.docx"
...
class TestNeverOverwrites:
    ...
    def test_numbering_continues(self, tmp_path):
        for name in ("report.docx", "report (1).docx", "report (2).docx"):
            (tmp_path / name).touch()
        assert output_path(tmp_path / "report.pdf", tmp_path, ".docx") == tmp_path / "report (3).docx"

    def test_overwrite_replaces_when_asked(self, make_pdf, tmp_path):
        pdf = make_pdf(tmp_path / "report.pdf")
        (tmp_path / "report.docx").write_bytes(b"old")
        result = convert(pdf, overwrite=True)
        assert result.output == tmp_path / "report.docx"
...
class TestRefusals:
    ...
    def test_a_folder(self, tmp_path):
        assert "is a folder, not a document" in self.refuse(tmp_path)

    def test_not_a_pdf(self, tmp_path):
        fake = tmp_path / "cv.pdf"
        fake.write_bytes(b"PK\x03\x04 a Word document, renamed")
        assert self.refuse(fake) == "cv.pdf is not a PDF or a Word document."

    def test_empty_file(self, tmp_path):
        empty = tmp_path / "empty.pdf"
        empty.touch()
        assert self.refuse(empty) == "empty.pdf is not a PDF or a Word document."
...
class TestDirection:
    """Which way a file converts is read out of the file, not off its name."""

    def test_a_pdf_goes_to_docx(self, make_pdf, tmp_path):
        path, conversion = check_input(make_pdf(tmp_path / "report.pdf"))
        assert conversion is PDF_TO_DOCX
        assert path.name == "report.pdf"

    def test_a_word_document_goes_to_pdf(self, make_docx, tmp_path):
        _, conversion = check_input(make_docx(tmp_path / "letter.docx"))
        assert conversion is DOCX_TO_PDF

    def test_a_pdf_named_docx_is_still_a_pdf(self, make_pdf, tmp_path):
        _, conversion = check_input(make_pdf(tmp_path / "misnamed.docx"))
        assert conversion is PDF_TO_DOCX

    def test_a_word_document_named_pdf_is_still_a_word_document(self, make_docx, tmp_path):
        _, conversion = check_input(make_docx(tmp_path / "misnamed.pdf"))
        assert conversion is DOCX_TO_PDF
